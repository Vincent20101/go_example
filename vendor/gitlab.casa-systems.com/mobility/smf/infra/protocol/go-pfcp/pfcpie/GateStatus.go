...
type GateStatus struct {
	UlGate uint8 `json:"ulGate,omitempty"` // 0x00001100
	DlGate uint8 `json:"dlGate,omitempty"` // 0x00000011
}
...
func (g *GateStatus) UnmarshalBinary(data []byte) error {
	...
	// Octet 5
	if length < idx+1 {
		return fmt.Errorf("IE GateStatus Inadequate TLV length: %d", length)
	}
