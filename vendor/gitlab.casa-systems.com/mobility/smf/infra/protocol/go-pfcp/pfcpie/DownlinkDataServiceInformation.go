...
type DownlinkDataServiceInformation struct {
	Qfii                        bool  `json:"qfii,omitempty"`
	Ppi                         bool  `json:"ppi,omitempty"`
	PagingPolicyIndicationValue uint8 `json:"pagingPolicyIndicationValue,omitempty"` // 0x00111111
	Qfi                         uint8 `json:"qfi,omitempty"`                         // 0x00111111
}
...
func (d *DownlinkDataServiceInformation) UnmarshalBinary(data []byte) error {
	...
	// Octet 5
	if length < idx+1 {
		return fmt.Errorf("IE DownlinkDataServiceInformation Inadequate TLV length: %d", length)
	}
	...
	if d.Ppi {
		if length < idx+1 {
			return fmt.Errorf("IE DownlinkDataServiceInformation Field Ppi Inadequate TLV length: %d", length)
		}
	...
	if d.Qfii {
		if length < idx+1 {
			return fmt.Errorf("IE DownlinkDataServiceInformation Field Qfii Inadequate TLV length: %d", length)
		}
