...
type RedirectInformation struct {
	RedirectAddressType         uint8  `json:"redirectAddressType,omitempty"` // 0x00001111
	RedirectServerAddressLength uint16 `json:"redirectServerAddressLength,omitempty"`
	RedirectServerAddress       []byte `json:"redirectServerAddress,omitempty"`
}
...
func (r *RedirectInformation) UnmarshalBinary(data []byte) error {
	...
	// Octet 5
	if length < idx+1 {
		return fmt.Errorf("IE RedirectInformation Inadequate TLV length: %d", length)
	}
	...
	// Octet 6 to 7
	if length < idx+2 {
		return fmt.Errorf("IE RedirectInformation Field RedirectServerAddressLength Inadequate TLV length: %d", length)
	}
	...
	if length < idx+r.RedirectServerAddressLength {
		return fmt.Errorf("IE RedirectInformation Field RedirectServerAddress Inadequate TLV length: %d", length)
	}
