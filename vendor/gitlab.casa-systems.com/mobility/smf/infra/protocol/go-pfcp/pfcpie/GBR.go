...
type GBR struct {
	UlGbr uint64 `json:"ulGbr,omitempty"` // 40-bit data
	DlGbr uint64 `json:"dlGbr,omitempty"` // 40-bit data
}
...
func (m *GBR) UnmarshalBinary(data []byte) error {
	...
	// Octet 5 to 9: UL GBR
	if length < idx+5 {
		return fmt.Errorf("Unmarshal IE GBR.[UL GBR] failed: Inadequate TLV length: %d", length)
	}
	tmpByteSlice := make([]byte, 8)
	...
	// Octet 10 to 14: DL GBR
	if length < idx+5 {
		return fmt.Errorf("unmarshal IE GBR [DL GBR ] failed: Inadequate TLV length: %d", length)
	}
	copy(tmpByteSlice[3:], data[idx:idx+5])
