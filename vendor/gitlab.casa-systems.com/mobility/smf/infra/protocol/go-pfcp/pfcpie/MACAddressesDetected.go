...
func (m *MACAddressesDetected) MarshalBinary() ([]byte, error) {
	var data []byte
	// Octet 5
...
func (m *MACAddressesDetected) UnmarshalBinary(data []byte) error {
	length := len(data)

	if len(data) < 1 {
		return fmt.Errorf("IE MACAddressesDetected Inadequate TLV length: %d", length)
	}
	...
	if numberOfMACAddresses >= 1 {
		if len(data) < numberOfMACAddresses*6 {
			return fmt.Errorf("IE MACAddressesDetected Field MACAddressValues Inadequate TLV length: %d", length)
		}
	...
	if ctagLength >= 1 {
		...
		if err := ctag.UnmarshalBinary(data[:ctagLength]); err != nil {
			return fmt.Errorf("IE MACAddressesDetected failed to unmarshal CTAG: %v", err)
		}
	...
	if stagLength >= 1 {
		...
		if err := stag.UnmarshalBinary(data[:stagLength]); err != nil {
			return fmt.Errorf("IE MACAddressesDetected failed to unmarshal STAG: %v", err)
		}
