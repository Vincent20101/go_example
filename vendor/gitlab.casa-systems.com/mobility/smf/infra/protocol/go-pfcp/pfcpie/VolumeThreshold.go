...
// VolumeThreshold IE. Refer to [TS29244 8.2.14 Time Threshold]
//
...
func (v *VolumeThreshold) UnmarshalBinary(data []byte) error {
	...
	// Octet 5
	if length < idx+1 {
		return fmt.Errorf("ie VolumeThreshold Inadequate TLV length: %d", length)
	}
	...
	if v.Tovol {
		if length < idx+8 {
			return fmt.Errorf("ie VolumeThreshold.TotalVolume Inadequate TLV length: %d", length)
		}
	...
	if v.Ulvol {
		if length < idx+8 {
			return fmt.Errorf("ie VolumeThreshold.UplinkVolume Inadequate TLV length: %d", length)
		}
	...
	if v.Dlvol {
		if length < idx+8 {
			return fmt.Errorf("ie VolumeThreshold.DownlinkVolume Inadequate TLV length: %d", length)
		}
