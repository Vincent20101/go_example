...
type DLBufferingDuration struct {
	TimerUnit  uint8 `json:"timerUnit,omitempty"`  // 0x11100000
	TimerValue uint8 `json:"timerValue,omitempty"` // 0x00011111
}
...
func (d *DLBufferingDuration) UnmarshalBinary(data []byte) error {
	...
	// Octet 5
	if length < idx+1 {
		return fmt.Errorf("IE DLBufferingDuration Inadequate TLV length: %d", length)
	}
