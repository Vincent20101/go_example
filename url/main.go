...
func main() {
	...
	fmt.Println(u.Host, u.Port(), u.Path, u.Hostname(), err)
	addr, err := net.ResolveIPAddr("ip4", "www.baidu.com")
	fmt.Println(addr, err)
	ip, err := net.LookupIP("www.baidu.com")
	fmt.Println(ip, err)
	hh, p, err := net.SplitHostPort("[::1]:80")
	//hh, p, err := net.SplitHostPort("smfsm.com:80")
	//hh, p, err := net.SplitHostPort("https://smfsm.com:80")  // error
